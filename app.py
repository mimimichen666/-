        ...
        if st.button("🍼 生成小白解读", help="把报告转写为通俗版：解释所有术语、"
            ...
            with st.expander("🍼 小白版报告（通俗解读）", expanded=True):
                ...
                plain = st.write_stream(llm_client.chat_stream(
                    [
                        {"role": "system", "content":
                         "你是一位优秀的科研科普编辑，擅长把学术论文写成"
                         "高中生也能读懂的科普文章。任务：把学术文献综述"
                         "转写成零基础读者也能完全看懂的通俗版长文。\n\n"
                         "核心原则（决定质量，逐条遵守）:\n"
                         "1. **术语解释必须真正展开**，不许敷衍。规则:\n"
                         "   - 术语首次出现时，用一段话(3句以上)完整讲解:\n"
                         "     它是什么 → 为什么被提出/解决什么问题 → "
                         "用一个生活化类比讲透原理\n"
                         "   - 禁止只换个说法复述（如原文'代理梯度'你写"
                         "'替代梯度法'就叫敷衍）；要把读者当成第一次"
                         "接触这个概念的人\n"
                         "   - 报告里出现的每个专业术语都要解释，宁多勿漏\n"
                         "2. **背景知识铺垫要厚**: 每个方法分类开头先用2-3段"
                         "讲清: 这个领域此前遇到了什么困难 → 这个方法的"
                         "核心直觉是什么(类比!) → 它聪明在哪里。像讲故事"
                         "一样有来龙去脉，不是罗列。\n"
                         "3. **教学性知识≠捏造事实**（重要区分）:\n"
                         "   - 允许且必须: 解释概念定义、原理、历史背景等"
                         "教科书式公共知识（这是科普写作，不是加事实）\n"
                         "   - 严禁: 编造报告之外的实验数据、性能数字、"
                         "论文结论、作者观点\n"
                         "   - 数据、结论、引用信息一律照抄报告原文\n"
                         "4. **结构要求**: 每个小节先'人话总结'(这个部分"
                         "一句话讲什么)再展开；段落多用类比和'为什么'\n"
                         "5. **篇幅不设上限**: 讲透优先，预计输出会比原"
                         "报告更长\n"
                         "6. markdown输出，标题口语化；文末加两块:\n"
                         "   - '关键术语表': 报告所有术语的一句话定义汇总\n"
                         "   - '读完后可以进一步了解什么'"},
                        {"role": "user",
                         "content": f"请转写以下文献综述（要求: 术语解释"
                                    "详尽展开、背景知识讲透、禁止复述式"
                                    "敷衍解释）:\n\n{report_md}"},
                    ],
                    temperature=0.3,
